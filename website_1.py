...
def get_pump2vacuum_data():
    global p2simpsi
    global pump2setpoint
    if pump2stat == '1' and pump2setpoint is not None:
        initial_value = 1
        if p2simpsi <= pump2setpoint:
             p2simpsi = p2simpsi + initial_value + random.randint(-2,4)
        elif p2simpsi <= 0 and p2simpsi > pump2setpoint:
             p2simpsi = p2simpsi - initial_value - random.randint(-2,5)
        elif p2simpsi > 0:
             p2simpsi = p2simpsi - initial_value - random.randint(-1,4)
        storedpsip2.append(p2simpsi)
        return jsonify({'pump2vacuum': p2simpsi})
    elif pump2stat == '1' and pump2setpoint is None:
        pump2setpoint = random.randint(-35,-30)
        initial_value = 1
        if p2simpsi <= pump2setpoint:
             p2simpsi = p2simpsi + initial_value + random.randint(-2,4)
        elif p2simpsi <= 0 and p2simpsi > pump2setpoint:
        ...
        if p2simpsi <= 6 and p2simpsi >= -6:
            ...
            p2simpsi = p2simpsi - random.randint(4,8)
            storedpsip2.append(p2simpsi)
            return jsonify({'pump2vacuum': p2simpsi})
        elif p2simpsi < -6:
            p2simpsi = p2simpsi + random.randint(4,8)
...
def get_pump3vacuum_data():
    global p3simpsi
    global pump3setpoint
    if pump3stat == '1' and pump3setpoint is not None:
        initial_value = 1
        if p3simpsi <= pump3setpoint:
             p3simpsi = p3simpsi + initial_value + random.randint(-2,4)
        elif p3simpsi <= 0 and p3simpsi > pump3setpoint:
             p3simpsi = p3simpsi - initial_value - random.randint(-2,5)
        elif p3simpsi > 0:
             p3simpsi = p3simpsi - initial_value - random.randint(-1,4)
        storedpsip3.append(p3simpsi)
        return jsonify({'pump3vacuum': p3simpsi})
    elif pump3stat == '1' and pump3setpoint is None:
        pump3setpoint = random.randint(-35,-30)
        initial_value = 1
        if p3simpsi <= pump3setpoint:
             p3simpsi = p3simpsi + initial_value + random.randint(-2,4)
        elif p3simpsi <= 0 and p3simpsi > pump3setpoint:
        ...
        if p3simpsi <= 6 and p3simpsi >= -6:
            ...
            p3simpsi = p3simpsi - random.randint(4,8)
            storedpsip3.append(p3simpsi)
            return jsonify({'pump3vacuum': p3simpsi})
        elif p3simpsi < -6:
            p3simpsi = p3simpsi + random.randint(4,8)
...
def get_pump4vacuum_data():
    global p4simpsi
    global pump4setpoint
    if pump4stat == '1' and pump4setpoint is not None:
        initial_value = 1
        if p4simpsi <= pump4setpoint:
             p4simpsi = p4simpsi + initial_value + random.randint(-2,4)
        elif p4simpsi <= 0 and p4simpsi > pump4setpoint:
             p4simpsi = p4simpsi - initial_value - random.randint(-2,5)
        elif p4simpsi > 0:
             p4simpsi = p4simpsi - initial_value - random.randint(-1,4)
        storedpsip4.append(p4simpsi)
        return jsonify({'pump4vacuum': p4simpsi})
    elif pump4stat == '1' and pump4setpoint is None:
        pump4setpoint = random.randint(-35,-30)
        initial_value = 1
        if p4simpsi <= pump4setpoint:
             p4simpsi = p4simpsi + initial_value + random.randint(-2,4)
        elif p4simpsi <= 0 and p4simpsi > pump4setpoint:
...
def get_pump5vacuum_data():
    global p5simpsi
    global pump5setpoint
    if pump5stat == '1' and pump5setpoint is not None:
        initial_value = 1
        if p5simpsi <= pump5setpoint:
             p5simpsi = p5simpsi + initial_value + random.randint(-2,4)
        elif p5simpsi <= 0 and p5simpsi > pump5setpoint:
             p5simpsi = p5simpsi - initial_value - random.randint(-2,5)
        elif p5simpsi > 0:
             p5simpsi = p5simpsi - initial_value - random.randint(-1,4)
        storedpsip5.append(p5simpsi)
        return jsonify({'pump5vacuum': p5simpsi})
    elif pump5stat == '1' and pump5setpoint is None:
        pump5setpoint = random.randint(-35,-30)
        initial_value = 1
        if p5simpsi <= pump5setpoint:
             p5simpsi = p5simpsi + initial_value + random.randint(-2,4)
        elif p5simpsi <= 0 and p5simpsi > pump5setpoint:
